// Set the BAYC and COOL contract addresses
...
const baycAbi = [
  ...
  { inputs: [], name: 'setStartingIndex', outputs: [], stateMutability: 'nonpayable', type: 'function' },
  {
    inputs: [],
    name: 'startingIndex',
    outputs: [{ internalType: 'uint256', name: '', type: 'uint256' }],
...
const getBaycOwners = async (totalSupply, baycContract) => {
  const baycAddresses = [];
  for (let i = 0; i < totalSupply; i++) {
    try {
      const baycTokenIdx = await baycContract.methods.tokenByIndex(i).call();
      const baycHolder = await baycContract.methods.ownerOf(baycTokenIdx).call();
      const baycBal = await baycContract.methods.balanceOf(baycHolder).call();
      baycAddresses.push(baycHolder);
      console.log(baycAddresses);
    } catch (error) {
      console.log(error?.message);
    }
  }
  return baycAddresses;
};

export { baycAbi, baycContractAddress, getBaycOwners };
